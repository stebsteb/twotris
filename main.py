...
import sys
import random
import twotris_settings as settings
import twotris_class as ttc
    ...
    def draw(self, screen):
        ...
        for y, row in enumerate(self.grid):
            for x, cell in enumerate(row):
                if cell:
                    pygame.draw.rect(screen, cell, (x * settings.GRID_SIZE, y * settings.GRID_SIZE, settings.GRID_SIZE - 1, settings.GRID_SIZE - 1))

        if self.current_piece:
            for i, row in enumerate(self.current_piece.shape[self.current_piece.rotation % len(self.current_piece.shape)]):
                for j, cell in enumerate(row):
                    if cell == 'O':
                        pygame.draw.rect(screen, self.current_piece.color, ((self.current_piece.x + j) * settings.GRID_SIZE, (self.current_piece.y + i) * settings.GRID_SIZE, settings.GRID_SIZE - 1, settings.GRID_SIZE - 1))

def draw_score(screen, score, x, y):
    """Draw the score on the screen"""
    font = pygame.font.Font(None, 36)
    text = font.render(f"Score: {score}", True, settings.WHITE)
...
def draw_game_over(screen, x, y):
    ...
    text = font.render("Game Over", True, settings.RED)
    screen.blit(text, (x, y))
...
def main():
    ...
    game = Tetris(settings.WIDTH // settings.GRID_SIZE, settings.HEIGHT // settings.GRID_SIZE)
    ...
    while True:
        ...
        if game.game_over:
            ...
            draw_game_over(screen, settings.WIDTH // 2 - 100, settings.HEIGHT // 2 - 30)  # Draw the "Game Over" message
